...
public class MergeSortPlusKT {
	...
	void mergeSort(int[] tempStorage, int lower, int upper) {
		...
		if (n < 20) {
			int[] sortedArray = insertionSort.insertionSort(Arrays.copyOfRange(theArray, lower, upper));
			System.arraycopy(sortedArray, 0, theArray, lower, sortedArray.length);
		} else {
	...
	public static void main(String[] args) {
		MergeSortPlusKT ms = new MergeSortPlusKT();
		// ms.testMerge();
		ms.testMergeSort();
	}
